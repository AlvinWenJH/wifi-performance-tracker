...
class PingMetricsCRUD:
    ...
    async def get_host_summary(self, host: str, hours: int = 24) -> Dict[str, Any]:
    ...
    async def delete_old_metrics(self, days: int = 30) -> int:
        ...
        try:
            ...
            logger.info(f"Deleted {deleted_count} old ping metrics (older than {days} days)")
            return deleted_count
        except Exception as e:
            logger.error(f"Failed to delete old metrics: {e}")
            raise
    
    async def get_metrics_count(self, host: Optional[str] = None, hours: Optional[int] = None) -> int:
